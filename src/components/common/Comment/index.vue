<template>
  <div id="comment" ref="comment">
    ...
    <!-- 底部评论列表 -->
    <comment-list v-if="cacheData.length > 0" ref="commentList">
      <!-- 单条评论 -->
      <comment-item
          ...
          <comment-list sub>
            ...
            >
              ...
              <comment-form
                  ...
                  :comment="child"
                  :parent="comment"
                  :placeholder="`回复${child.user && child.user.username}...`"
                  :upload-img="uploadImg"
                  @form-delete="deleteForm"
                  @form-submit="formSubmit"
              />
...
export default {
  ...
  created() {
    // 监听并执行一次
    const cancel = this.$watch('comments', () => {
      this.processData()
      cancel && cancel()
    })
  },
  ...
  methods: {
    ...
    processData() {
      //todo 直接复制丢失头像？
      //this.cacheData = this.comments
      //console.log(this.cacheData)
      this.cacheData = this.comments.map(this.comparePropsAndValues)
      //console.log(this.cacheData)
    ...
    comparePropsAndValues(comment) {
      ...
      for (const key in originObj) {
        originObj[key] = comment[this.commentModel[key]] || comment[key] || originObj[key]
    ...
    hasForm(id) {
      ...
      // vue3 todo 不支持$children,并且会自动触发其他表单blur事件？
      //this.broadcastBlur(this.$refs, id)
    },
    // * 增加新表单
    addForm(id) {
      this.forms.push(id)
    ...
    addComment(id, rawData) {
      ...
      this.$nextTick(() => {
        // 滚动至可见视图上
        const signal =
            commentIndex === 'root' ? this.cacheData.length - 1
                : `${commentIndex}-${this.cacheData[commentIndex].children.length - 1}`
    ...
    scrollIntoView(ref) {
      this.$nextTick(() => {
        this.$refs[ref].$el.scrollIntoView(false)
      })
    }
  }
}
...
#comment {
  ...
  padding-top: 1.0664em;

  & > .comment-form {
    margin: 0 1.3328em 1.0664em;
  }

  & > .comment-list {
    margin: 0 1.3328em 0 5.2em;
